...
| §04 Deck generation | `chart::mint_deck` — decan→pip (cardinal 2–4 / fixed 5–7 / mutable 8–10), chart-ruler→Ace, angular/ruling→court by dignity, plus each planet's Major Arcana; stats from degree/minute/dignity |
...
| §07 Star → zone tug-of-war | `resolve_star_battle` + `strike_star_single` + `apply_control` (signed meter, flip at ±600; instant single-card Drag & Drop progressive strikes) |
...
| GPS engagement | `unity/GpsService.cs` (single GPS authority, with editor fallback) → `set_location` (private `player_location`); `resolve_star_battle` & `strike_star_single` gate on `altitude_deg ≥ 10°`. `SkyRenderer` dims the 0–10° band and `BattlePanel` disables Strike with the reason, so the AR view matches the gate |
| Deck curation | `set_loadout` (Active capped at 8) via a per-card loadout chip in `DeckPanel`/`CardView` (Active → Defense → Bench); `deploy_card` converts cards into `Loadout::Defense` sentinel garrisons to defend held stars against rival raids |
...
- **Instant Drag & Drop Progressive Raids**: Drag any card from your active hand directly onto a star node or zone to chip away at its control threshold in real time (`strike_star_single` / `strikeStarSingleLive`). Features suit weather alignment (+35% affinity bonus) and magnitude resistance scaling.
...
*   **Pillar pools**: `pillar_pool.esms` uses the same unit as f64, starting from the player's jing pool (or 80 units). A pillar cast charges 10 units, and the daily faucet credits 10 units per 1.0 ESMS granted, keeping the fraction that the jing credit floors.
