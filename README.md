...
*   **The Wanderers on Their Own Plane**: The ten bodies (real geocentric positions, the same low-precision ephemeris as `feeder/ephemeris.ts`) ride the **ecliptic arc**, drawn dashed across the star field on a layer above it. Planet discs are deliberately exaggerated — faction-coloured glyph medallions with halos — so they are never lost among the stars, which stay pinprick-small until hovered (they bloom under the cursor). Tap a planet to select the zone it is transiting; its tooltip gives sign, degree and altitude.
