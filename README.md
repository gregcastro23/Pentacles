...
| Blend (natal × transits) | `chart::house_cusps` / `house_of` / `house_salience` lay equal-house cusps under the live sky; `chart::blended_faction_vector` adds each transiting body's (house salience × sky-dignity) onto the natal `faction_scores`, so a transit crossing onto an angle can open or close a faction round to round. `ecliptic_lon_arcmin` recovers each transit's zodiac λ from its stored RA/Dec |
