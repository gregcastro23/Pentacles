...
// `pillar_pool.esms` uses the `jing_pool.esms` unit — tenths of an ESMS — so a
// new pillar pool can copy the jing row 1:1, and the circuit's baseline and cast
// charge (80 and 10) are 8.0 and 1.0 ESMS. It is f64, so pillar credits keep the
// sub-unit remainder that jing floors away.
const _: () = assert!(JING_STARTER_ESMS as f64 == alchm_astro_core::circuit::POOL_BASELINE);

/// Pure helper: determine initial pillar pool ESMS from optional pre-existing jing pool or the starter balance.
pub fn compute_initial_pillar_pool(jing_esms: Option<&[u16]>) -> alchm_astro_core::circuit::Esms {
    let starter = JING_STARTER_ESMS;
    match jing_esms {
        Some(jp) => [0, 1, 2, 3].map(|k| jp.get(k).copied().unwrap_or(starter) as f64),
        None => [starter as f64; 4],
    }
}
...
/// Pure arithmetic: credit a faucet grant to a pillar pool, in pool units.
///
/// Uses the same 10^4 fixed-point amount the `faucet_transaction` row records,
/// converted without flooring: 1.0000 ESMS adds exactly 10.0 units.
pub fn compute_faucet_pillar_credit(
    ...
    let fixed_points = allocation.to_fixed_points();
    let per_unit = ESMS_ATOMS_PER_POOL_UNIT as f64;
    [0, 1, 2, 3].map(|k| current_pools[k] + fixed_points[k] as f64 / per_unit)
}
...
pub fn compute_faucet_pool_credit(
    ...
    let mut new_jing = jing
        .map(|j| j.to_vec())
        .unwrap_or_else(|| vec![JING_STARTER_ESMS; 4]);
    credit_pool_fixed_points(&mut new_jing, &allocation.to_fixed_points());
...
fn apply_duel_outcome_to_initiator(
    ...
    let init_pool = ensure_pillar_pool(ctx, initiator);
    let init_esms: alchm_astro_core::circuit::Esms = [
        initiator_pools.get(0).copied().unwrap_or(80.0),
        initiator_pools.get(1).copied().unwrap_or(80.0),
...
pub fn counter_pillar(
    ...
    apply_duel_outcome_to_initiator(ctx, duel.initiator, &duel.initiator_pools, &outcome.pools_a);
...
pub fn cancel_pillar_duel(ctx: &ReducerContext, duel_id: u64) -> Result<(), String> {
    ...
    // Refund initiator's escrowed charge without rolling back subsequent duels
    refund_initiator_escrow(ctx, duel.initiator, &duel.initiator_pools);
...
mod tests {
    ...
    #[test]
    fn test_initiator_escrow_and_duel_outcome_delta_preserves_subsequent_duels() {
        let initial_pools = [80.0, 80.0, 80.0, 80.0];
        let q = alchm_astro_core::circuit::CAST_CHARGE;
        
        // When cast_pillar is called, charge is paid from baseline
        let paid = alchm_astro_core::circuit::pay(&initial_pools, q);
        let escrow = [
            initial_pools[0] - paid[0],
            initial_pools[1] - paid[1],
            initial_pools[2] - paid[2],
            initial_pools[3] - paid[3],
        ];
        assert_eq!(escrow[0], 0.0);
        assert_eq!(escrow[1], 0.0);
        assert!((escrow[2] + escrow[3] - 10.0).abs() < 1e-6);

        // Player's pool after cast
        let pool_after_cast = paid;
        assert_eq!(pool_after_cast[2], 75.0);
        assert_eq!(pool_after_cast[3], 75.0);

        // Subsequent duels occur (e.g. 11 duels against Mars), draining pool:
        ...
        assert!((refunded_pool[3] - (0.0 + escrow[3])).abs() < 1e-6);
        assert!(refunded_pool[2] < 15.0, "Escrow refund must not restore Matter to 80.0");
    ...
    fn test_daily_faucet_allocation_refills_pillar_pool() {
        ...
        // 6.0 ESMS is 60 pool units.
        assert_eq!(refilled[0], 140.0);
        assert_eq!(refilled[1], 140.0);
        assert!((refilled[2] - 64.8f64).abs() < 1e-6);
        assert_eq!(refilled[3], 60.0);
    ...
    fn test_faucet_pool_credit_new_player_seeds_default_then_credits_once() {
        // Both pools start at the 80-unit starter; the grant is 30/25/40/25 units.
        let (pillar, jing) = super::compute_faucet_pool_credit(None, None, &uneven_grant());
        assert_eq!(pillar, [110.0, 105.0, 120.0, 105.0]);
    ...
    fn test_faucet_pool_credit_existing_pillar_only_adds_grant() {
        ...
        assert_eq!(pillar, [40.0, 45.0, 44.75, 25.0]);
        assert_eq!(jing, vec![110, 105, 120, 105]);
    }
    ...
    fn test_faucet_pool_credit_seeds_missing_pillar_from_pre_grant_jing() {
        ...
        assert_eq!(pillar, [110.0, 120.0, 100.0, 145.0]);
    ...
    fn test_faucet_pool_credit_pads_short_pillar_row_from_pre_grant_jing() {
        ...
        assert_eq!(pillar, [80.0, 75.0, 140.0, 135.0]);
    }

    #[test]
    fn test_faucet_pool_credit_pillar_keeps_the_fraction_jing_floors() {
        // 2.3456 ESMS is 23.456 units: jing floors to 23, pillar keeps all of it.
        ...
        let (pillar, jing) = super::compute_faucet_pool_credit(None, None, &grant);
        assert!((pillar[0] - 103.456).abs() < 1e-9);
        assert_eq!(pillar[1..], [110.0, 110.0, 110.0]);
        assert_eq!(jing, vec![103, 110, 110, 110]);
    }
}
