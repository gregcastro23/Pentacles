...
pub fn create_player(
    ...
    // round tally and arm exactly one round timer at the deck's current interval.
...
pub fn push_ephemeris(
    ...
    let row = Ephemeris { body, ra, dec, transiting_zone, retrograde, tick: ctx.timestamp };
...
/// Live transits for the blend: every body in the ephemeris, reduced to its ecliptic
/// longitude (recovered from RA/Dec) and retrograde flag.
fn gather_transits(ctx: &ReducerContext) -> Vec<chart::TransitSnapshot> {
    let mut transits: Vec<chart::TransitSnapshot> = ctx
        .db
        .ephemeris()
        .iter()
        .map(|e| chart::TransitSnapshot {
            body: e.body,
            lon_arcmin: chart::ecliptic_lon_arcmin(e.ra, e.dec),
            retrograde: e.retrograde,
        })
        .collect();
    // Fixed order (by body) so an identical sky always blends identically, whatever
    // the table scan order — the draft must be reproducible from the snapshot alone.
    transits.sort_by_key(|t| t.body.idx());
    transits
}
...
fn draft_one(ctx: &ReducerContext, owner: Identity, chart_row: &NatalChart, round_index: u64) {
    let transits = gather_transits(ctx);
    if transits.is_empty() {
        return; // the sky hasn't been seeded yet — nothing to draft from
    }
    let Some(spec) = chart::draft_card(chart_row, &transits, round_index, owner, hero_ceiling(ctx, owner))
    else {
        return;
    };
...
mod tests {
    use super::{
        auto_battle_win, catchup_rounds, pick_weakest, question_hash, round_interval_secs,
        should_replace, COLLECTION_CAP, MAX_CATCHUP_ROUNDS, ROUND_BASE_SECS,
    };
